...
class Locator{
  constructor(){
    this._regions = ["Ústecký", "Jihočeský", "Jihomoravský", "Karlovarský", "Královéhradecký",
    "Kraj Vysočina", "Liberecký", "Moravskoslezský", "Olomoucký", "Pardubický", "Plzeňský",
    "Praha", "Stredočeský", "Zlínský"];
    this._dataset = gdal.open("scripts/service/resources/CZE_adm1.shp");
  }
  
  /**
   * Find the region where the point specified by received latitude and longitude belongs
   * @param {number} latitude
   * @param {number} longitude
   * @return {string} region - name of the region. Return null if not found.
   */
  find(latitude, longitude){
    var point = new gdal.Point(longitude, latitude);
    var region = null;
